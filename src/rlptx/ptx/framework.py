...
class PtxSystem:
    ...
    def next_step(self, category_attributes):
        ...
        self.current_step += 1
        self.update_all_tracked_attributes(category_attributes)
        old_balance = self.previous_balance
        self.previous_balance = self.balance
        return self.balance - old_balance
    
    def update_all_tracked_attributes(self, category_attributes):
        ...
        for commodity in self.get_all_commodities():
            commodity.update_tracked_attributes(category_attributes["commodity"])
        for generator in self.get_generator_components_objects():
            generator.update_tracked_attributes(category_attributes["generator"])
        for conversion in self.get_conversion_components_objects():
            conversion.update_tracked_attributes(category_attributes["conversion"])
